...
.kingdom-focused-ui .kingdom-nav::-webkit-scrollbar,.kingdom-focus-summary::-webkit-scrollbar,.kingdom-focus-actions::-webkit-scrollbar{display:none}
...
.kingdom-focus-metric span{display:block;color:#76604a;font:700 .52rem 'Cinzel',serif;text-transform:uppercase}
...
@media(max-width:760px){
 ...
 .kingdom-focused-ui .kingdom-nav{top:-6px!important;padding:4px!important;border-radius:9px!important}
 ...
 .kingdom-focus-metric{padding:6px 7px}.kingdom-focus-metric span{font-size:.47rem}.kingdom-focus-metric strong{font-size:.76rem}
 .kingdom-focus-alert{min-height:31px;padding:5px 7px;font-size:.62rem}.kingdom-focus-alert>span{width:20px;height:20px;flex-basis:20px;font-size:.65rem}
 .kingdom-focus-actions{min-height:33px;gap:5px}.kingdom-focus-actions-label{font-size:.49rem}.kingdom-focus-action{min-height:31px!important;padding:5px 8px!important;font-size:.52rem!important}
 .kingdom-focused-ui .kingdom-command-center{grid-template-columns:1fr!important}
 .kingdom-focused-ui .kingdom-score{display:grid!important;grid-template-columns:1fr auto auto!important;padding:8px!important;align-items:center!important}.kingdom-focused-ui .kingdom-score strong{font-size:1.25rem!important}
 .kingdom-focused-ui .kingdom-advisor{padding:8px!important}.kingdom-focused-ui .kingdom-advisor-head span{font-size:.67rem!important}.kingdom-focused-ui .kingdom-advisor-head small{font-size:.52rem!important}.kingdom-focused-ui .kingdom-advisor ol{padding-left:17px!important}
 .kingdom-focused-ui .manager-section{padding:8px!important;margin-bottom:6px!important}.kingdom-focused-ui .manager-section-head{min-height:29px!important;margin-bottom:6px!important}.kingdom-focused-ui .manager-section-head h4{font-size:.7rem!important}.kingdom-focused-ui .manager-section-head>span{font-size:.56rem!important}
 .kingdom-focused-ui .manager-kpi-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:5px!important}.kingdom-focused-ui .manager-kpi{padding:6px!important}
 .kingdom-focused-ui .manager-compact-card{grid-template-columns:minmax(0,1fr)!important;gap:4px!important;padding:8px!important}.kingdom-focused-ui .kingdom-social-card,.kingdom-focused-ui .kingdom-faction-card{padding:8px!important}
 .kingdom-focused-ui .kingdom-filterbar{overflow-x:auto!important;display:flex!important;flex-wrap:nowrap!important;gap:5px!important}.kingdom-focused-ui .kingdom-filterbar>*{flex:0 0 auto!important;min-width:125px!important}
}
@media(max-width:390px){.kingdom-focused-ui .manager-hero p{display:none!important}.kingdom-focused-ui .manager-cash strong{max-width:42vw}.kingdom-focused-ui .kingdom-nav button{font-size:.5rem!important;padding-inline:7px!important}.kingdom-focus-summary{grid-auto-columns:minmax(90px,1fr)}}`;
    ...
    function buildSummaryData(value) {
        const kingdom = value && typeof value === 'object' ? value : {};
        const people = kingdom.people || {};
        const crises = asArray(kingdom.crises).filter(item => !item?.status || /active|attiv|open|ongoing/i.test(String(item.status)));
        const hostile = asArray(kingdom.factions).filter(item => number(item?.hostility) >= 65 || /ostil|nemic|avvers|guerra|rival/i.test(`${item?.relationship || ''} ${item?.stance || ''}`));
        const metrics = [
            { label: 'Tesoro', value: Math.round(number(kingdom.treasury)).toLocaleString('it-IT'), tone: number(kingdom.treasury) < 0 ? 'critical' : 'normal' },
            { label: 'Stabilità', value: `${Math.round(clamp(kingdom.stability, 0, 100))}%`, tone: number(kingdom.stability, 50) < 40 ? 'warning' : 'normal' },
            { label: 'Consenso', value: `${Math.round(clamp(people.approval, 0, 100))}%`, tone: number(people.approval, 50) < 40 ? 'warning' : 'normal' },
            { label: 'Pressioni', value: String(crises.length + hostile.length), tone: crises.length || hostile.length ? 'warning' : 'good' }
        ];
        let alert = { tone: 'good', icon: '✓', text: 'Nessuna emergenza prioritaria rilevata.' };
        const severe = crises.slice().sort((a, b) => number(b?.severity) - number(a?.severity))[0];
        if (severe && number(severe.severity) >= 55) alert = { tone: 'critical', icon: '⚠', text: `Crisi prioritaria: ${clean(severe.name || severe.title || severe.description || 'crisi del regno', 140)}` };
        else if (number(kingdom.treasury) < 0) alert = { tone: 'critical', icon: '⚠', text: 'Tesoro negativo: ridurre costi o creare nuove entrate.' };
        else if (number(people.unrest) >= 55) alert = { tone: 'critical', icon: '⚠', text: `Disordini popolari al ${Math.round(number(people.unrest))}%.` };
        else if (number(kingdom.food) <= 0 && number(kingdom.population) > 0) alert = { tone: 'critical', icon: '⚠', text: 'Riserve alimentari esaurite.' };
        else if (number(kingdom.stability, 50) < 40) alert = { tone: 'warning', icon: '!', text: `Stabilità fragile: ${Math.round(number(kingdom.stability, 50))}%.` };
        else if (hostile.length) alert = { tone: 'warning', icon: '⚑', text: `${hostile.length} fazion${hostile.length === 1 ? 'e avversaria' : 'i avversarie'} da monitorare.` };
        return { metrics, alert, activeCrises: crises.length, hostileFactions: hostile.length };
    }
    ...
    function assignSectionGroups(rootEl) {
        ...
        sections.forEach(section => {
            ...
            section.dataset.kingdomFocusGroup = classified || current || 'kingdom-overview';
    ...
    function ensureSummary(rootEl) {
        ...
        if (!nav) return;
        const summary = buildSummaryData(getState()?.worldMemory?.kingdom);
        const metricsHtml = summary.metrics.map(item => `<article class="kingdom-focus-metric ${escapeHtml(item.tone)}"><span>${escapeHtml(item.label)}</span><strong>${escapeHtml(item.value)}</strong></article>`).join('');
        const metricSignature = summary.metrics.map(item => `${item.label}:${item.value}:${item.tone}`).join('|');
        let metrics = rootEl.querySelector('.kingdom-focus-summary');
        if (!metrics) {
            metrics = rootEl.ownerDocument.createElement('section');
            metrics.className = 'kingdom-focus-summary';
            metrics.setAttribute('aria-label', 'Indicatori principali del regno');
            nav.after(metrics);
        }
        if (metrics.dataset.signature !== metricSignature) {
            metrics.dataset.signature = metricSignature;
            metrics.innerHTML = metricsHtml;
        }

        const alertSignature = `${summary.alert.tone}|${summary.alert.icon}|${summary.alert.text}`;
        let alert = rootEl.querySelector('.kingdom-focus-alert');
        if (!alert) {
            alert = rootEl.ownerDocument.createElement('div');
            metrics.after(alert);
        }
        alert.className = `kingdom-focus-alert ${summary.alert.tone}`;
        if (alert.dataset.signature !== alertSignature) {
            alert.dataset.signature = alertSignature;
    ...
    function ensureActionPanel(rootEl) {
        ...
        let panel = rootEl.querySelector('.kingdom-focus-actions');
        if (!buttons.length) {
            if (panel) panel.classList.add('kingdom-focus-hidden');
            return;
        }
        if (!panel) {
            panel = rootEl.ownerDocument.createElement('div');
            panel.className = 'kingdom-focus-actions';
            const alert = rootEl.querySelector('.kingdom-focus-alert');
            (alert || source).after(panel);
        }
        panel.classList.remove('kingdom-focus-hidden');
        const signature = activeSection + '|' + buttons.map(button => `${button.dataset.kingdomAction}:${button.dataset.service || ''}:${clean(button.textContent, 80)}:${button.disabled}`).join('|');
    ...
    function setSectionVisibility(rootEl) {
        rootEl?.querySelector('#kingdom-overview')?.classList.toggle('kingdom-focus-hidden', activeSection !== 'kingdom-overview');
        ...
        sections.forEach(section => {
            ...
            if (visibleIndex > 0) {
                ...
                const collapsed = collapsedSections.has(collapseKey) ? collapsedSections.get(collapseKey) : true;
        ...
        const columns = rootEl?.querySelector('.manager-columns');
        if (columns) columns.classList.toggle('kingdom-focus-hidden', !sections.some(section => !section.classList.contains('kingdom-focus-hidden')));
    }
    ...
    function updateNav(rootEl) {
        rootEl?.querySelectorAll('.kingdom-nav [data-kingdom-section]').forEach(button => {
            ...
            if (label && clean(button.textContent, 80) !== label) button.textContent = label;
    ...
    function applyFocus(rootEl, options = {}) {
        if (!rootEl?.querySelector('.kingdom-nav')) return false;
        ...
        if (options.scrollTop) rootEl.closest('.modal-body')?.scrollTo?.({ top: 0, behavior: 'smooth' });
        return true;
    }

    function bindEvents(doc) {
        if (!doc || doc.documentElement?.dataset.kingdomFocusUiBound === '1') return;
        doc.documentElement.dataset.kingdomFocusUiBound = '1';
        doc.addEventListener('click', event => {
            ...
            if (navButton) {
                ...
                activeSection = next;
                applyFocus(doc.getElementById('kingdom-dashboard'), { scrollTop: true });
                return;
            }
            ...
            const collapseKey = section?.dataset?.kingdomCollapseKey;
            if (!section || !collapseKey) return;
            section.classList.toggle('is-collapsed');
            collapsedSections.set(collapseKey, section.classList.contains('is-collapsed'));
        }, true);
    }
    ...
    function scheduleRefresh(doc) {
        ...
        const run = () => { refreshScheduled = false; refresh(doc); };
    ...
    function observe(doc) {
        ...
        const target = doc.getElementById('kingdom-dashboard') || doc.documentElement || doc.body;
