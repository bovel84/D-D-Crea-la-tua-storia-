(function (root, factory) {
    ...
    function detectPresentationContext(state = getState()) {
        ...
        const historical = era !== 'modern' || /storico|rinasc|medioev|firenze|venezia|roma antica/.test(corpus);
    ...
    function contextualRole(state = getState(), context = detectPresentationContext(state)) {
        ...
        if (context.historical) {
            ...
            return context.era === 'renaissance' ? 'Cittadino della Firenze rinascimentale' : 'Personaggio storico';
        }
        const labels = { ceo: 'CEO', founder: 'Fondatore', analyst: 'Analista', controller: 'Controller', seller: 'Venditore', fixer: 'Intermediario', manager: 'Manager', coder: 'Sviluppatore', investigator: 'Investigatore', striker: 'Attaccante', playmaker: 'Regista', defender: 'Difensore', goalkeeper: 'Portiere' };
    ...
    function profileModel(state = getState()) {
        ...
        return {
            context,
            name: clean(state.character.name, 140), house, role, background, place,
            period: [eraLabel(context), context.year].filter(value => value !== null && value !== '').join(' · '),
    ...
    function cssText() {
        return `
        #modal-character .modal{width:min(680px,calc(100vw - 18px));max-height:min(94dvh,920px);overflow:hidden;border-radius:24px;border:1px solid rgba(161,111,25,.38);box-shadow:0 28px 80px rgba(30,16,8,.42)}
        #modal-character .modal-header{min-height:64px;padding:14px 18px;color:#f5dc8a;background:linear-gradient(135deg,#2b160d,#170b07 74%);border-bottom:1px solid rgba(224,184,72,.45)}
        #modal-character .modal-body{padding:16px;background:linear-gradient(180deg,#f8e9bd,#f2dda7)}
        #modal-character .char-header{position:relative;margin:0 0 14px;padding:18px 14px 15px;border-radius:20px;border:1px solid rgba(154,107,25,.18);background:rgba(255,251,235,.48);box-shadow:0 9px 24px rgba(76,46,18,.06)}
        #modal-character .char-header:before{content:'';position:absolute;inset:0;pointer-events:none;border-radius:20px;background:radial-gradient(circle at 50% -10%,rgba(198,150,47,.18),transparent 48%)}
        #modal-character #char-portrait{width:156px!important;height:156px!important;margin:0 auto 12px!important;border-radius:32px!important;border:2px solid rgba(123,79,25,.42)!important;box-shadow:0 14px 30px rgba(48,29,14,.17)!important}
        ...
        #${IDENTITY_ID}{position:relative;display:flex;flex-wrap:wrap;justify-content:center;gap:7px;margin-top:11px}
        #${IDENTITY_ID} .profile-chip{display:inline-flex;align-items:center;min-height:30px;padding:5px 9px;border-radius:999px;color:#5b4328;background:rgba(255,255,255,.6);border:1px solid rgba(112,77,31,.13);font:700 .7rem system-ui,sans-serif}
        #modal-character .profile-world-card{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:0 0 14px;padding:12px;border-radius:16px;background:rgba(255,251,237,.55);border:1px solid rgba(133,92,30,.13)}
        ...
        #modal-character[data-profile-theme='renaissance'] .profile-world-card,#modal-character[data-profile-theme='medieval'] .profile-world-card{border-left:4px solid rgba(142,96,20,.58)}
        @media(max-width:620px){#modal-character{align-items:flex-end}#modal-character .modal{width:100%;max-width:none;max-height:calc(100dvh - 16px);margin:0;border-radius:24px 24px 0 0}#modal-character .modal-body{padding:13px 12px calc(18px + env(safe-area-inset-bottom))}#modal-character #char-portrait{width:148px!important;height:148px!important;border-radius:30px!important}#modal-character .char-header{padding:15px 10px 13px}#modal-character .profile-world-card{grid-template-columns:1fr 1fr;padding:10px}}
        `;
    }
    ...
    function updatePresentationIdentity(state, model) {
        ...
        if (model.context.historical && (!clean(character.role) || /^(ceo|startupper|founder|manager)$/i.test(clean(character.role)))) character.role = model.role;
    ...
    function equipmentIsEmpty(documentRef) {
        ...
        return Boolean(slots.length) && slots.every(slot => /vuoto|empty/i.test(clean(slot.textContent, 120)));
    }
    ...
    function render(documentRef, state = getState()) {
        ...
        if (!identity && cls) {
            identity = documentRef.createElement('div'); identity.id = IDENTITY_ID; cls.insertAdjacentElement('afterend', identity);
        }
        if (identity) {
            const chips = [model.house ? `🏛 ${model.house}` : '', model.place ? `📍 ${model.place}` : '', model.period ? `📜 ${model.period}` : ''].filter(Boolean);
        ...
        let card = body?.querySelector('.profile-world-card');
        if (!card && body) {
            card = documentRef.createElement('div'); card.className = 'profile-world-card';
            if (equipment) equipment.insertAdjacentElement('beforebegin', card); else body.prepend(card);
        }
        if (card) {
            const rows = [['Ruolo nel mondo', model.role], ['Origine sociale', model.background], ['Casata / famiglia', model.house || 'Nessuna casata nota'], ['Epoca e luogo', [model.period, model.place].filter(Boolean).join(' · ') || 'Contesto corrente']];
            card.innerHTML = rows.map(([label, value]) => `<div class="profile-world-item"><small>${escapeHtml(label)}</small><strong>${escapeHtml(value)}</strong></div>`).join('');
        }
    ...
    function install(documentRef, windowRef) {
        ...
        if (!documentRef.__characterProfileV2Installed) {
            ...
            documentRef.addEventListener('click', event => {
                if (event.target?.closest?.('#btn-top-character,#btn-character')) windowRef.setTimeout(() => render(documentRef, getState()), 0);
            }, true);
    ...
    return { PATCH_VERSION, STYLE_ID, IDENTITY_ID, clean, keyOf, detectPresentationContext, contextualRole, contextualBackground, eraLabel, houseLabel, profileModel, cssText, updatePresentationIdentity, refreshPortraitProfile, equipmentIsEmpty, render, install };
});